...
def _parse_args():
    ...
    # init-db
    init_db_cmd = subparsers.add_parser("init-db", help="Initialize/verify database schema")
    init_db_cmd.add_argument("--db", default="pesa_logger.db", help="Database path")

    # serve
    # NOTE: API key is loaded exclusively from .env / environment variables.
    # Do NOT pass secrets via CLI flags — they appear in shell history and process lists.
    ...
    # export-excel
    excel_cmd = subparsers.add_parser("export-excel", help="Export transactions to Excel")
    ...
    # insights
    insights_cmd = subparsers.add_parser("insights", help="Show AI-generated financial insights")
    ...
    # summary
    summary_cmd = subparsers.add_parser("summary", help="Show weekly or monthly financial summary")
    summary_cmd.add_argument("--period", choices=["weekly", "monthly"], default="monthly")
    ...
    corpus_cmd.add_argument("--path", default="corpus/mpesa_sms_corpus.jsonl")
    corpus_cmd.add_argument("--min-success", type=float, default=0.98)
    ...
    correct_cmd.add_argument("--transaction-id", required=True)
    correct_cmd.add_argument("--set", action="append", required=True)
    correct_cmd.add_argument("--reason", required=True)
    correct_cmd.add_argument("--by", default="cli")

    # list-corrections
    list_corr_cmd = subparsers.add_parser("list-corrections", help="List correction audit records")
    list_corr_cmd.add_argument("--db", default="pesa_logger.db")
    list_corr_cmd.add_argument("--transaction-id", default=None)
    list_corr_cmd.add_argument("--limit", type=int, default=100)

    # list-inbox
    list_inbox_cmd = subparsers.add_parser("list-inbox", help="List raw inbox SMS rows")
    list_inbox_cmd.add_argument("--db", default="pesa_logger.db")
    list_inbox_cmd.add_argument("--limit", type=int, default=200)
    list_inbox_cmd.add_argument("--oldest-first", action="store_true")
    list_inbox_cmd.add_argument(
        "--parse-status",
        default=None,
        choices=["pending", "success", "failed", "duplicate"],
    )
    list_inbox_cmd.add_argument("--sim-slot", default=None)

    # list-transactions
    list_tx_cmd = subparsers.add_parser("list-transactions", help="List canonical transactions")
    list_tx_cmd.add_argument("--db", default="pesa_logger.db")
    list_tx_cmd.add_argument("--limit", type=int, default=200)
    list_tx_cmd.add_argument("--type", default=None)
    list_tx_cmd.add_argument("--category", default=None)
    list_tx_cmd.add_argument("--sim-slot", default=None)

    # reparse-failed
    reparse_cmd = subparsers.add_parser("reparse-failed", help="Re-run parser on failed inbox rows")
    reparse_cmd.add_argument("--db", default="pesa_logger.db")
    reparse_cmd.add_argument("--limit", type=int, default=1000)

    # verify-ledger
    verify_ledger_cmd = subparsers.add_parser("verify-ledger", help="Verify ledger chain integrity")
    verify_ledger_cmd.add_argument("--db", default="pesa_logger.db")

    # ledger-events
    ledger_events_cmd = subparsers.add_parser("ledger-events", help="List ledger chain events")
    ledger_events_cmd.add_argument("--db", default="pesa_logger.db")
    ledger_events_cmd.add_argument("--limit", type=int, default=200)
    ledger_events_cmd.add_argument("--entity-table", default=None)

    # rebuild-ledger
    rebuild_ledger_cmd = subparsers.add_parser("rebuild-ledger", help="Backfill ledger chain")
    rebuild_ledger_cmd.add_argument("--db", default="pesa_logger.db")
    rebuild_ledger_cmd.add_argument("--force", action="store_true")

    # failed-report
    failed_report_cmd = subparsers.add_parser("failed-report", help="Classify failed inbox messages")
    failed_report_cmd.add_argument("--db", default="pesa_logger.db")
    failed_report_cmd.add_argument("--limit", type=int, default=5000)
    failed_report_cmd.add_argument("--sample-size", type=int, default=3)
    failed_report_cmd.add_argument("--sim-slot", default=None)
...
def main():
    ...
    if args.command == "init-db":
        print(json.dumps({"status": "ok", "message": "Database schema initialized/verified", "db": args.db}, indent=2))

    elif args.command == "sms":
        from pesa_logger.ingestion import ingest_sms_text
        result = ingest_sms_text(sms_text=args.text, db_path=args.db, source="cli")
        ...
        # API key is loaded from .env / environment only — never from CLI flags.
        api_key = os.environ.get("PESA_API_KEY", "").strip()
        if not api_key:
            print(
                "WARNING: PESA_API_KEY is not set. All authenticated endpoints are unprotected.\n"
                "         Set PESA_API_KEY in your .env file before exposing this server.",
                file=sys.stderr,
            )

        app = create_app(db_path=args.db, api_key=api_key or None)
        ...
        data = weekly_summary(db_path=args.db) if args.period == "weekly" else monthly_summary(db_path=args.db)
        print(json.dumps(data, indent=2))

    elif args.command == "heartbeat":
        from pesa_logger.monitoring import heartbeat_status
        result = heartbeat_status(db_path=args.db, threshold_hours=args.hours, record=True)
        print(json.dumps(result, indent=2))
        if result["alert"]:
            sys.exit(2)
        ...
        output = backup_database(db_path=args.db, backup_dir=args.backup_dir, keep_last=args.keep_last)
        ...
        result = validate_corpus(path=args.path, min_success_rate=args.min_success)
        print(json.dumps(result, indent=2))
        ...
        rows = list_transaction_corrections(
            db_path=args.db, transaction_id=args.transaction_id, limit=args.limit
        )
        ...
        result = reparse_failed_inbox_sms(db_path=args.db, limit=args.limit)
        print(json.dumps(result, indent=2))
        ...
        rows = list_ledger_events(db_path=args.db, limit=args.limit, entity_table=args.entity_table)
        print(json.dumps(rows, indent=2))

    elif args.command == "rebuild-ledger":
        from pesa_logger.database import rebuild_ledger_chain
        result = rebuild_ledger_chain(db_path=args.db, force=args.force)
        print(json.dumps(result, indent=2))
        if result.get("status") == "skipped":
            sys.exit(1)
        if not (result.get("verification") or {}).get("valid", True):
            sys.exit(2)
